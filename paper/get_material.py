...
def save2CSV(dishes_dict):
    w =csv.writer(open("elements.csv","w"))#,encoding="utf-8"))
...
def getAreaElementsMeanAmount(area,dishes_dict):
    ...
    for key,val in area_dishes.iteritems():
        ...
        for key1,val1 in val[3].iteritems():
            katakana_key=m.parse(str(key1)).replace("\n","")
            if area_elements.has_key(str(key1)):
                area_elements[str(key1)][0]+=1
                area_elements[str(key1)][1]+=val1[0]
            else:
                area_elements[str(key1)]=[1,1]
                
            if area_elements1.has_key(katakana_key):
                area_elements1[katakana_key][0]+=1
                area_elements1[katakana_key][1]+=val1[0]
            else:
                area_elements1[katakana_key]=[1,1]
                      
    area_elements2={}
    for element,val in area_elements.iteritems():
        katakana_key=m.parse(element).replace("\n","")
        
        if katakana_key!=element:
            #print type(katakana_key)
            area_elements1[katakana_key][0]=-1
            area_elements1.update({element:val})
    
    for element,val in area_elements1.iteritems():
        if val[0]!=-1:
            #print element+"\t"+str(val[1]/val[0])
            area_elements2.update({element:val[1]/val[0]})
    
    return area_elements2
...
def getAreaElements(area,dishes_dict):
    ...
    for key,val in area_dishes.iteritems():
        ...
        for key1,val1 in val[3].iteritems():
            katakana_key=m.parse(str(key1)).replace("\n","")
            area_elements[str(key1)]=area_elements.setdefault(str(key1),0)+1      
            #area_elements1[katakana_key]=area_elements1.setdefault(katakana_key,0)+1
      
    area_elements2={}
    for element,freq in area_elements.iteritems():
        katakana_key=m.parse(element).replace("\n","")
        
        if katakana_key!=element:
            #print type(katakana_key)
            area_elements1[katakana_key]=-1
            area_elements1.update({element:freq})
    
    for element,freq in area_elements1.iteritems():
        if freq!=-1:
            area_elements2.update({element:freq})
...
def tfidf(area, dishes_dict):
    ...
    for element,val in elements_tf.iteritems():
        #print element
        #if elements_idf.has_key()
        tfidf_value=val*elements_idf[element]
        if elements_all_mean[element]!=0:
            #print elements_area_mean[element]/elements_all_mean[element]
            tfidf_value=tfidf_value*elements_area_mean[element]/elements_all_mean[element]  
